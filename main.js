function love(param) {
  ...
  body.style.background = "rgb(63, 81, 181)";
...
const startText = () => {
  ...
  setTimeout(() => {
    document.querySelector('.vmeste').style = "display: flex;"
...
const startNew = () => {
  ...
  document.querySelector('.buttonStart').innerHTML = ""


  setTimeout(() => {
    document.querySelector('.text1').innerHTML = "Моя красотка"
  }, 5000)

  setTimeout(() => {
    document.querySelector('.text2').innerHTML = "Люблю тебя"
  }, 10000)


  setTimeout(() => {
    document.querySelector('.newStart').style = "display:none;"
  }, 15000)
...
const arrStartText = () => {
  ...
  document.querySelector('.arrText').style = "display:flex;"
  
  setInterval(() => {
    
    if (number == 30) {
      document.querySelector('.arrText').innerHTML = 'Люблю тебя'
    }
    
    if (number == 45) {
      document.querySelector('.arrText').innerHTML = 'Это бесконечные сердечки 😜'
    }
    
    if (number == 55) {
      document.querySelector('.arrText').innerHTML = 'Вот еще смайлик 🥰'
    }
    
    if (audio.currentTime > 132) {
      document.querySelector('.arrText').innerHTML = 'Все песня кончилась расходимся, целую'
    }
    
    document.querySelector('.arrText').innerHTML += ` ${arrTextLive[number++]} `
  }, 1000)
}